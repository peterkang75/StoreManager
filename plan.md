...
| `school_holidays` | `id` (serial PK), `name`, `startDate`, `endDate`, `createdAt` | Global school holiday (break) periods (~4 per year, **NSW**). Not per-store. Drives automatic Term-vs-Holiday season detection for roster hour caps (week = HOLIDAY when ≥4 of 7 days fall in a range). Bundled NSW dataset loadable via `POST /api/store-config/school-holidays/load-nsw`. |
...
| `store_recommended_hours` | `storeId` (PK, FK → stores), `termWeeklyHours` (real), `holidayWeeklyHours` (real), `updatedAt` | LEGACY advisory weekly hours (term vs holiday). Superseded at roster time by `store_hour_caps`; retained as the seed source. |
| `store_hour_caps` | `id` (PK), `storeId` (FK), `season` (`'TERM'`\|`'HOLIDAY'`), `weeklyTotalHours` (real), `saturdayHours` (real), `sundayHours` (real), `publicHolidayHours` (real, per PH day), `updatedAt` | **Enforced** per-store, per-season staff-hour caps for rostering. Unique index `(store_id, season)`. Weekday pool = weeklyTotal − sat − sun − phPerDay×#PH (fixed). Seeded from `store_recommended_hours` on migration. Edited via Store Settings "Hour Caps" tab; enforced at roster Publish. |
...
- **Manager-facing screens = English only (2026-06-21)**: Any screen a MANAGER role can see (e.g. Roster Builder `Rosters.tsx`, Pending Approvals `TimesheetApprovals.tsx`) must contain NO Korean — not even guidance/subtitle text. Managers don't read Korean. This overrides the "admin guidance in Korean" rule above FOR shared pages reachable by managers. When a string differs by role, show English to MANAGER (e.g. `currentRole === "MANAGER" ? "English" : "한국어"`). Owner/ADMIN-only screens (e.g. Store Settings) may keep Korean guidance.
...
- **Season-based roster hour caps — Phase 1 (2026-06-18)**: Per-store, per-season enforced staff-hour caps for rostering. New table `store_hour_caps(store_id, season['TERM'|'HOLIDAY'], weekly_total_hours, saturday_hours, sunday_hours, public_holiday_hours)`, seeded from the old advisory `store_recommended_hours`. Pure math in `shared/rosterCaps.ts` (season classify = **majority of 7 days vs `schoolHolidays`**; PH precedence over weekend; weekday pool = total − sat − sun − phPerDay×#PH, fixed). Server single-source `getWeekCaps(storeId, weekStart)` (routes.ts) classifies the week, resolves PH days (excl. store closures) + caps; used by both `GET /api/rosters/week-caps` (builder display) and **publish enforcement** (`POST /api/rosters/publish` → 403 `CAP_EXCEEDED` for non-ADMIN when any category over cap; ADMIN bypasses). Cap is **store-total staff-hours**, enforced at **Publish** (drafts free). **A sub-cap of 0 = "no limit for that category" (not "zero allowed")** — so leaving Sat/Sun/PH at 0 enforces only the weekly total across all days (the common case); the weekday pool is enforced only when a weekend/PH carve-out exists (any sub-cap > 0). Weekly total always enforced when > 0. (Updated 2026-06-21 after the original "0 = zero allowed" blocked normal weekend rosters.) Config UI: StoreConfig "Recommended Hours" tab reworked to "Hour Caps" (`GET/PUT /api/store-config/hour-caps`). Season data: NSW school-holiday breaks bundled in `shared/nswSchoolHolidays.ts` (2025–2027, Eastern/Sydney; Autumn-2026 boundary worth re-checking vs official calendar), loaded via `POST /api/store-config/school-holidays/load-nsw` button + manual edit; "Victoria"→"NSW" label fixed. Builder (`Rosters.tsx`) shows season badge + per-category used/cap (red over) and disables Publish when over. Spec: `docs/superpowers/specs/2026-06-18-roster-hour-caps-design.md`; plan: `docs/superpowers/plans/2026-06-18-roster-hour-caps.md`. **Phase 2 (pending):** recommend `weekly_total_hours` from prior-week `dailySales` × target labour% ÷ blended wage.
- **Timesheet-approval deadline + Director override (2026-06-18)**: Formalizes "마감" for *manager timesheet approvals* as an automatic, time-based lock (separate from payroll save-close and employee submission window). Deadline = **end of the Monday following the cycle's Sunday end** ("그 다음 월요일 자정"); cycle locks at `sydneyToday >= shiftDate(cycleEnd, 2)` (helpers `getApprovalDeadlineMonday`/`getApprovalLockDate`/`isCycleApprovalClosed` in `shared/payrollCycle.ts`, all Australia/Sydney). **Server-enforced** (not UI-only): all manager mutations on `/api/admin/approvals/*` (approve, edit-approve, update-times, reject, delete, add-shift, auto-fill, bulk-approve, bulk-revert) call `approvalLockBlock()` → 403 `CYCLE_LOCKED` for MANAGER when closed & no override. **ADMIN (Director) bypasses always** — edits without unlocking. Director override = `timesheet_approval_overrides(store_id, cycle_start)` (row exists = that store+cycle re-opened); manual re-lock deletes it. ADMIN-only endpoints `unlock-cycle`/`relock-cycle`; `GET cycle-status` feeds the Approve-screen banner. Frontend `TimesheetApprovals.tsx` (already cycle-based): deadline banner + red locked-state ("Director에게 연락") + amber override-active + ADMIN unlock/relock toggle; Add Shift / Auto-Fill / Approve All disabled when locked. Landing cycle is conditional: defaults to the previous cycle while its window is still open (finalizing the just-ended fortnight), but once that cycle locks it defaults to the current open cycle so managers don't hit the locked wall by default. The deadline is a cutoff, NOT a one-day window — managers approve freely throughout the fortnight + the buffer Monday; only past it does the lock engage. Spec: `docs/superpowers/specs/2026-06-18-timesheet-approval-deadline-design.md`. Addresses the Susan case (06-01~14 shifts approved 06-17 after 06-16 close): managers now see the deadline and are blocked post-close; late shifts still flow via back-pay.
- **Past-period payroll unlock (2026-06-16)**: Saved past periods are read-only in the UI (`isLocked = selectedRow.payrollId && periodEnd < today`, `Payrolls.tsx`). This protects closed books but had NO escape hatch when a save was wrong (e.g. bad Cash/Gross split). Added an explicit per-period "Unlock to edit" button (state `unlockedCtxKey`, keyed by `${storeId}|${start}|${end}`): admin unlocks → edits → normal `Save All Payroll`. The bulk-save path already re-syncs ALL derived records (CASH_WAGE upsert by ref-note, intercompany settlements, bank deposits) from the payroll rows, so re-saving is the correct fix — never hand-delete the CASH_WAGE cashflow row (it's a derived mirror; deleting only desyncs the ledger and gets recreated on next save). Re-locks automatically on navigate-away (ctxKey mismatch) and on successful save (`onSuccess` resets `unlockedCtxKey`). Frontend-only — the PUT/bulk APIs never enforced the lock. Multi-store corrections: unlock + re-save each affected store separately. NOT YET BUILT: single-click undo/revert (no payroll snapshot/history table exists; updatePayroll overwrites in place).
- **Aggregate revenue reads `daily_sales`, never a SUM over `daily_closings` (2026-08-11)**: `daily_closings` is append-only with **no uniqueness on (store_id, date)** — a re-submitted close form leaves both rows, so any `SUM` double-counts that day. Two such days exist in production (Sandwich 2026-06-08 and 2026-07-30; the 07-30 pair was submitted 70 seconds apart). `daily_sales` carries `daily_sales_store_date_uniq` on (store_id, date) and is upserted by `storage.mirrorClosingToDailySales`, so last-submission-wins is already applied; it also covers POSnet-imported days that never had a close form. `daily_sales.total` = POS sales + Uber Eats + DoorDash — identical to the "Total Income" column on Cash & Close. The P&L (`/api/reports/pnl`) and the Attendance History revenue card both read it via `GET /api/daily-sales`. Verified: the two sources agree on every store-day except the two duplicated ones. **Caveat on the upsert:** last-write-wins can silently drop figures the earlier submission had — the 2026-06-08 re-entry lost an $87.70 Uber Eats amount. The Cash & Close screen's own weekly totals row still sums `daily_closings` and is therefore inflated on those two days.
- **Scheduled times come from `rosters`, never `shifts` (2026-07-28)**: `shifts` is a legacy table (FK-bound to `rosterPeriods`) with **0 rows in production**; the Roster Builder writes only to `rosters` (2,828 rows). `GET /api/admin/approvals` had been enriching every timesheet from `getShifts()`, so the `Scheduled` and `Diff` columns on **both** Attendance History and Pending Approvals rendered `—` for every row, always — not a data gap, a wrong join. Fixed to `getRosters()`, indexed by `employeeId|storeId|date` (the store MUST be in the key — an employee can be rostered at both stores on the same day; verified in production). If any future feature needs a planned-vs-actual comparison, read `rosters`. Do not add rows to `shifts`.
- **Aggregate roster-vs-actual diffs must come from the hour totals, not from summing per-cell diffs (2026-07-28)**: A rostered day with no timesheet (no-show) has no per-cell diff — there is nothing to subtract from. Summing cell diffs therefore silently drops the missed hours: during verification an employee rostered 36h who worked 19h15m reported `+1h 15m` instead of `−16h 45m`, i.e. the view built to catch no-shows hid them. Always compute `hoursDiffMinutes(actualHours, rosterHours)` (`client/src/components/attendance/attendanceModel.ts`) for any row, week, day, or page-level total. Per-cell diffs remain 0 for no-show/unscheduled cells — those cells communicate via their own status treatment instead.
- **Attendance History views (2026-07-28)**: `client/src/pages/admin/Timesheets.tsx` is a shell; the comparison views live in `client/src/components/attendance/` (`attendanceModel.ts` = pure merge logic + the canonical date/time helpers, `AttendanceGridView.tsx`, `AttendanceTimelineView.tsx`). Rows are the **union** of employees with a roster and employees with an approved timesheet — keying off timesheets alone hides the rostered-but-never-worked employee. Store filtering happens at **entry** level before grouping, so a dual-store employee's per-store totals stay correct. Grid is read-only by design: Rosters' `CellEditor` is the write path and is deliberately not reused; approved records are edited only via Revert to Pending.
...
- **Envelope pay slips print as fixed A4 thirds (2026-08-11)**: `PaySlips.tsx` no longer lets content decide where the tear line falls. Slips are chunked 3-per-sheet in JS (`chunkIntoSheets`) into a `210mm × 296.9mm` `.payslip-sheet`, each holding three `.payslip-slot`s of fixed `height` (99mm / 99mm / 98.9mm, `box-sizing: border-box`, `overflow: hidden`). The dashed rule therefore always prints at **exactly 99mm and 198mm from the top of the paper**, on every sheet, so a stack can be guillotined in one cut. Load-bearing details, each of which breaks the feature if changed: (1) sheet height is **296.9mm, not 297mm** — Chrome's mm→device-px rounding on an exact-height child otherwise emits a blank page between every sheet (the 0.1mm is absorbed by slot 3 so slots 1–2 stay exact); (2) `height`, never `min-height` — one growing slot would push both lines down; (3) the last sheet is padded with **empty slots** so a partial page still carries the same cut lines; (4) `break-after: page` on every sheet **except `:last-child`**, else a trailing blank page. Free text is the only overflow risk, so the adjustment reason was **taken out of the table entirely** (2026-08-11, same day): the money table is 6 columns (Store/Hours/Gross/Adjustment/Cash/Bank) and the reason renders below the Total line in its own bordered `.payslip-reason` box of fixed two-line height (`height: 2.6em` + `-webkit-line-clamp: 2`), with the Total Cash / Total Bank box beneath it. Fixed height is the point — the summary box sits at the same offset whatever the reason says, and a long reason clamps instead of pushing money rows out of a clipped slot. Reasons are per-store but the box is per-slip, so `renderReasons()` prefixes the store code only when more than one store carries a reason. **Printed wording is deliberately terse and de-identified** (2026-08-11): Store→`Location`, Gross→`Total`, Cash (Env)→`Envelope`, Bank Deposit→`Deposit`, and the two summary labels are just `Envelope` / `Deposit`. Store names print as **codes, not names** — `storeCode()` maps Sushi→`SS`, Sandwich→`SW` and passes any other store through under its own name (an unmapped label beats a wrong one). This mapping exists only on the printed slip; every other screen still shows store names. **Depends on the operator printing at 100% scale with no browser margins/headers** (any "fit to page" scaling shifts every line); the on-screen bar states this. Verified 2026-08-11 by rendering a 4-slip fixture to PDF with headless Chrome and measuring the rasterised pages at 150dpi: 4 slips → 2 pages (no blanks), A4 MediaBox (594.96 × 841.92pt), and full-paper-width dashed rules on **identical pixel rows of both pages** — top-of-border measured at 98.72mm and 197.61mm, i.e. the 1px border occupying the last 0.26mm of each fixed slot, so its bottom edge is the 99mm / 198mm cut position. Cross-sheet registration (identical rows on every page) is the property that matters here, not the absolute figure.
- **Cash Sales Entry 자동저장 — 자기 저장이 유발한 refetch는 그리드를 재구성하지 않는다 (2026-09-08)**: `CashSalesEntry.tsx`는 타이핑 1초 후 `POST /api/cash-sales/bulk`로 자동저장하는데, 이 엔드포인트는 해당 기간을 통째로 삭제 후 재삽입하므로 (1) 저장 성공 → `invalidateQueries` → refetch → populate effect가 그대로 돌면 저장 왕복 중에 친 글자가 날아간다. 그래서 저장 성공 시 `skipRowResetUntilRef = Date.now() + 5000` 윈도우를 세우고, populate effect는 **같은 store|period일 때만** 그 윈도우 안에서 `setRows`를 건너뛴다(전환 시엔 항상 재구성 — 플래그가 영구히 남아 정상 재구성을 잡아먹는 것을 막기 위해 boolean이 아닌 시한부 타임스탬프). void(삭제) 성공 시엔 윈도우를 0으로 리셋해 반드시 재구성한다. (2) in-flight 중에는 새 저장을 시작하지 않는다(삭제/재삽입 인터리브 방지) — 저장 중 들어온 입력은 `editSeq` 비교로 dirty를 유지해 다음 디바운스에 다시 저장된다. (3) 예약된 저장이 전환 후의 다른 가게/기간에 착지하지 않도록 `dirtyCtxRef`(store|periodStart) 키를 타이머 발화 시점에 검증한다. (4) blur 저장은 **포커스가 그리드 밖으로 나갈 때만** — 셀 간 Tab마다 저장하면 기간 전체 재작성이 셀 수만큼 발생한다.
...
- [x] **Cash Sales Entry 자동저장 + 메모 저장 버그** ✅ COMPLETE (2026-09-08)
  - **버그**: `POST /api/cash-sales/bulk`가 클라이언트가 보낸 `memo`를 insert payload에서 누락 → 저장 버튼을 눌러도 메모가 DB에 저장되지 않았음 (`server/routes.ts` bulk 핸들러, `MemStorage.createCashSalesDetail`도 동일 누락).
  - **오해 정정**: 숫자칸이 새로고침 후 살아있던 것은 저장돼서가 아니라 `daily_close_forms`에서 매번 재구성되기 때문. 자동저장은 원래 존재하지 않았음.
  - **자동저장 구현** (`client/src/components/CashSalesEntry.tsx`): 타이핑 멈춤 1초 후 디바운스 저장, 그리드 밖으로 포커스 이동/가게·기간 전환 시 즉시 flush. 기존 Save 버튼은 수동 즉시 저장으로 유지(토스트는 수동 저장만).
  - **안전장치**: (1) 자기 자신의 저장이 유발한 refetch가 입력 중인 그리드를 덮어쓰지 않도록 5초 skip 윈도우 — 같은 store/period일 때만 적용, (2) bulk 엔드포인트가 기간 전체를 삭제 후 재삽입하므로 in-flight 중 중복 저장 금지(직렬화), (3) 저장 중 들어온 입력은 editSeq 비교로 dirty 유지 → 재저장, (4) 예약된 저장이 전환된 다른 가게/기간에 착지하지 않도록 컨텍스트 키 가드, (5) 미저장 상태에서 탭 닫기/새로고침 시 beforeunload 경고.
  - **날짜 없는 행 경고**: 저장 payload는 `date`가 있는 행만 보내므로, 데이터만 있고 날짜가 없는 행은 날짜칸 빨간 테두리 + 상단 경고 문구로 표시(기존에는 조용히 유실).
  - 메모만 있고 금액이 0인 행도 reload 시 유지되도록 `isMeaningfulCashSale`에 memo 포함.
  - **검증**: 빌드 통과, memo 라운드트립을 로컬 DB(storage 레이어)에서 확인. 브라우저 E2E는 로컬 admin 계정이 없어 미실시 — Railway 배포 후 실사용 확인 필요.
...
| Phase 10 | Timesheet-approval deadline + Director override; Past-period payroll unlock; Back-pay payment-vs-cost (2026-06-18) | §5 notes |
| Phase 11 | Season-based roster hour caps — Phase 1: enforced per-store/season caps, auto NSW-season detection, publish-time enforcement (2026-06-18) | §5 note; `docs/superpowers/plans/2026-06-18-roster-hour-caps.md` |
| Phase 12 | Attendance History: store filter + roster-vs-actual Grid/Timeline views; fixed the dead `shifts`→`rosters` scheduled-time join (2026-07-28) | §5 note; `docs/superpowers/specs/2026-07-28-attendance-roster-comparison-design.md` |
...
#### 6.3.-1 Roster Hour Caps — Phase 2: sales-based recommendation 📋
- **Status:** Deferred (Phase 1 shipped 2026-06-18). 
- **What:** Recommend each store's `weekly_total_hours` cap from the prior week's `dailySales` × a target labour-cost % ÷ the store's blended hourly wage (`employee_store_assignments.rate`), surfaced as a one-click "apply" into `store_hour_caps`.
- **Prereq:** Phase 1 (`store_hour_caps` table + `getWeekCaps`) — done. Needs a target labour-% setting (per store) and a compute endpoint.
