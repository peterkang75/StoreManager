...
- **Shift Presets (Settings)**: Admin Settings → Shift Presets at `/admin/settings/shift-presets`. Per-store configurable time presets for the three quick-fill buttons (Full Day, Open Shift, Close Shift) shown in the Roster cell editor. Data stored in `shift_presets` table (id, storeId, fullDayStart, fullDayEnd, openShiftStart, openShiftEnd, closeShiftStart, closeShiftEnd). Only Sushi and Sandwich stores are shown (roster-enabled stores). API: `GET /api/shift-presets`, `GET /api/shift-presets/:storeId`, `PUT /api/shift-presets/:storeId` (upsert). CellEditor uses preset values when available, falls back to calculated half-day from store hours if no preset exists.
