...
- **Multi-Store Employees**: `employeeStoreAssignments` junction table supports employees working at multiple stores. Import de-duplicates by first+last name and creates per-store assignments with store-specific rate/fixedAmount. Payroll grid shows all employees assigned to the selected store (via assignments or direct storeId). Payroll records include `storeId` to track which store a payroll entry belongs to.
