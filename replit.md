...
- **Payroll**: Spreadsheet-like payroll grid with real-time calculations (Calculated Amount, Gross/Cash reciprocal, PAYG tax via ATO Schedule 1 fortnightly coefficients, Super 11.5%, Bank Deposit). Supports fortnightly period auto-detection (Mon-Sun two-week blocks), bulk save, CSV/TSV employee import, persistent employee memos, and global store payroll notes. CashBalances and ConvertForm are pinned at top (sticky) for quick cash management during payroll processing.
...
- **Cash Sales Entry**: Dedicated page at `/admin/cash-sales` with sidebar link under Finance. Has Entry tab (spreadsheet-like grid for bulk-inputting 2-week cash denomination counts per store, auto-calculates counted amounts from denominations $100-5c, shows discrepancy vs envelope amounts in red) and History tab (past period lookup with summary card and detail modal). Creates a CASH_SALES financial transaction on save. Finance page also shows detail modal for CASH_SALES transactions via eye icon. Component: `CashSalesEntry.tsx` (shared), page: `CashSales.tsx`.
