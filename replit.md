...
- **Cash Sales Entry**: Spreadsheet-like grid in Finance tab for bulk-inputting 2-week cash denomination counts per store. Auto-calculates counted amounts from denominations ($100-5c), shows discrepancy vs envelope amounts in red, and creates a CASH_SALES financial transaction on save to update the store's cash balance. Reference note includes store name for multi-store isolation.
