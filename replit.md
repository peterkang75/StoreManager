...
- **Employee Portal (`/m/portal`, Phase 4)**: Mobile-first self-service portal for Sushi/Sandwich store employees to submit daily timesheets. 3-step login flow: store selection → employee name → 4-digit PIN (via numpad). Dashboard shows today's scheduled shift (if the roster week is published). Employee can "Confirm Scheduled Hours" (submits timesheet matching the roster exactly) or "Modify Hours" (opens a drawer with 15-min-increment selectors + mandatory reason field). `shiftTimesheets` table (id, storeId, employeeId, date, actualStartTime, actualEndTime, status PENDING/APPROVED/REJECTED, adjustmentReason). Session persisted in `sessionStorage`. API routes: `GET /api/portal/stores`, `GET /api/portal/employees`, `POST /api/portal/login`, `GET /api/portal/shift`, `GET /api/portal/timesheet`, `POST /api/portal/timesheet`.
